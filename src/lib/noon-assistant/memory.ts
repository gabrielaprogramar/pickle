import type { NoonMemoryEntry } from "./types";

export interface NoonMemory {
  remember(vesselId: string, key: string, value: string): void;
  recall(vesselId: string, key: string): string | null;
  list(vesselId: string): ReadonlyArray<NoonMemoryEntry>;
...
export function createNoonMemory(): NoonMemory {
  const store = new Map<string, NoonMemoryEntry[]>();

  function remember(vesselId: string, key: string, value: string): void {
    const entries = store.get(vesselId) ?? [];
    const timestamp = new Date().toISOString();
    const existing = entries.find((e) => e.key === key);
    let next: NoonMemoryEntry[];
    if (existing) {
      next = entries.map((e) =>
  ...
  function list(vesselId: string): ReadonlyArray<NoonMemoryEntry> {
